...
function GetJsonMessageResponse($RequestMessageType,$EchoJArray)
{
	...
	if( $RequestMessageType == "IntentRequest" ){
		...
		{
			$NextNumber = $EchoJArray->session->attributes->countActionList->currentStep + 1;
			$SpeakPhrase = "Thers is 3 subjects: English, read the degrees of comparison and articles for test. Environmental Science,complete the book activity in page no 175. Maths, draw some patterns which you have found around in yourself in A4 sheet";
		...
		{
			/$NextNumber = $EchoJArray->session->attributes->countActionList->currentStep + 1;
			$SpeakPhrase = "Dear Parents, Tomorrow (23.01.2019) will be a full working day till 4.00 pm (Regular yellow uniform) following Wednesday Timetable. Regards, My School Guru";
			
		}
		if( $EchoJArray->request->intent->name == "fee collection" )
		{
			/$NextNumber = $EchoJArray->session->attributes->countActionList->currentStep + 1;
